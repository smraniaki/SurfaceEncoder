...
def calculate_metrics(original_images, reconstructed_images):
    ...
    for original, reconstructed in zip(original_images, reconstructed_images):
        ...
        s = ssim(
            ...
            data_range=original_reshaped.max() - original_reshaped.min(),
            multichannel=False,
        )
...
mse_values, psnr_values, ssim_values = calculate_metrics(
...
for i in range(n):
    ...
    original_img = (
        original_images[i].reshape(128, 128)
        if original_images[i].shape[-1] == 1
    ...
    reconstructed_img = (
        reconstructed_images[i].reshape(128, 128)
        if reconstructed_images[i].shape[-1] == 1
        else reconstructed_images[i]
    )
    plt.imshow(
        reconstructed_img,
        cmap="gray" if reconstructed_images[i].shape[-1] == 1 else None,
    )
    ax.get_xaxis().set_visible(False)
    ax.get_yaxis().set_visible(False)
plt.show()
