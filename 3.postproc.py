...
class AutoencoderDemo(QWidget):
    ...
    def filterAndThresholdImage(self):
        try:
            ...
            img = np.frombuffer(buffer, dtype=np.uint8).reshape((qPixMap.height(), qPixMap.width(), 4))
            ...
            bytesPerLine = width
            qImg = QImage(final_image.data, width, height, bytesPerLine, QImage.Format_Grayscale8)
            
            # Scale the image to match the scaling used in updateImage
            qImg = qImg.scaled(width * 1, height * 1, Qt.KeepAspectRatio)  # Adjust the scaling factor if necessary
